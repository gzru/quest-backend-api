...
class CheckSignViewQuery(Query):
...
class CheckSignViewSession(object):

    def __init__(self, global_context):
        self._signs_engine = SignsEngine(global_context)

    def parse_query(self, data):
        self._query = CheckSignViewQuery()
        self._query.parse(data)

    def execute(self):
        found = self._signs_engine.check_views(self._query.user_id, [self._query.sign_id])
