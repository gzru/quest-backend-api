...
class GrantSignAccessQuery(Query):
...
class GrantSignAccessSession(object):

    def __init__(self, global_context):
        self._signs_engine = SignsEngine(global_context)

    def parse_query(self, data):
        self._query = GrantSignAccessQuery()
        self._query.parse(data)

    def execute(self):
        self._signs_engine.grant_sign_access(self._query.user_id, self._query.sign_id)
